...
/* ─── Track last assistant message for suggestions ───────────── */
...
const suggestionBtn = document.getElementById("suggestion_btn");
...
async function sendMessage() {
    ...
    // Hide suggestion box when user sends a message
    hideSuggestion();
    ...
    // Save the last assistant message for suggestion feature
    lastAssistantMessage = result;
...
// Click suggestion box → paste into input
suggestionBox.addEventListener("click", () => {
    if (suggestionBox.textContent) {
...
suggestionBtn.addEventListener("click", async () => {
    if (!lastAssistantMessage) {
        showSuggestion("No question yet — wait for the assistant to speak first.");
        return;
    }
    ...
    try {
        ...
        while (true) {
            ...
            for (const line of lines) {
                try {
                    ...
                    if (obj.message?.content) {
                        result += obj.message.content;
                        suggestionBox.textContent = result;
                    }
                } catch(e) {}
            }
        }
        ...
        showSuggestion("Could not generate suggestion. Please try again.");
    }
});
...
if (selectedTopic) {
    ...
    // No topic — send a default greeting so assistant speaks first
    (async () => {
        const response = await fetch("http://localhost:8000/chat", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ message: "Hello! Please greet me and ask me a simple English speaking question to get us started." })
        });
